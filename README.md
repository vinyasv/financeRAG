...
# Ingest a document
python3 scripts/ingest.py annual_report.pdf

# Ask questions
python3 scripts/query.py "What was total revenue for Q4 2024?"
```
...
**Document Processing**
- PDF table extraction: Gemini VLM → Docling local fallback
...
- Suspicious prompt-pattern advisory logging with input sanitization
...
```bash
python3 scripts/ingest.py report.pdf                # Single file
python3 scripts/ingest.py report.pdf data.xlsx      # Multiple files
python3 scripts/ingest.py -f ./earnings/            # Entire folder
python3 scripts/ingest.py -f ./reports --pattern "*.pdf"
```
...
```bash
python3 scripts/query.py                                 # Interactive mode
python3 scripts/query.py "What was 2024 revenue growth?" # Single query
python3 scripts/query.py -m claude-sonnet "..."          # Choose model
python3 scripts/query.py -o report.pdf "..."             # Export to PDF
python3 scripts/query.py --list-models                   # Available models
```
...
| `EMBEDDING_PROVIDER` | `local` | `auto` (prefers OpenRouter if key available), `local` (BAAI/bge-small-en-v1.5), or `openrouter` |

## Architecture

```
src/
├── agent/              # Planner, DAG executor, ExecutionMonitor, synthesizer
├── cli/                # Shared CLI helpers for logging, exports, and output formatting
├── common/             # Shared IDs, naming, prompts, and text rendering helpers
...
├── llm_client.py       # OpenRouter LLM abstraction (model aliases)
...
tests/                  # 11 test suites covering security, tools, storage, execution, and refactors
```
...
| Table extraction | Gemini 2.5 Flash Lite VLM → [Docling](https://github.com/DS4SD/docling) TableFormer (local) |
